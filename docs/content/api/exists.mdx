...
**Async.** Returns `Promise<boolean>`. This is a Rush-FS convenience API; Node does not expose `fs.promises.exists`.
...
- **Unsupported Node path inputs**: `Buffer` paths and `URL` paths are not part of the current Rush-FS `exists` surface.
