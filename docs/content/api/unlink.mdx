...
- **Directory code gap**: Directory unlink failures are platform-specific in Node; Rush-FS currently reports `EPERM` text for directories.
- **Unsupported Node path inputs**: `Buffer` paths and `URL` paths are not part of the current Rush-FS `unlink` surface.
